...
bool CSceneNode::InitModel(const std::string modelname, const std::string tex)
{
	// Call the parent's Init()
	CSolidObject::Init();
	...
	// load and create a texture 
	iTextureID = CImageLoader::GetInstance()->LoadTextureGetID(tex.c_str(), false);
	if (iTextureID == 0)
	{
		cout << "Unable to load " << tex << endl;
		return false;
	}
...
void CSceneNode::CheckandLoadModel(const std::string modelname, const std::string tex)
{
...
bool CSceneNode::Update(const double dElapsedTime)
{
	...
	//vec3Front = -(CPlayer3D::GetInstance()->GetPosition() - localTransformMtx[3].x);
	//if (glm::length(vec3Front) <= 10) {
	//	float angle = atan2(-vec3Front.z, vec3Front.x);

	//	glm::vec3 front;
	//	front.x = cos(glm::radians(fYaw)) * cos(glm::radians(0.f));
	//	front.y = sin(glm::radians(0.f));
	//	front.z = sin(glm::radians(fYaw)) * cos(glm::radians(0.f));

	//	angle = atan2(-front.z, front.x);
	//	std::vector<CSceneNode*>::iterator it;
	//	for (it = vChildNodes.begin(); it != vChildNodes.end(); ++it) { // update the child of the parent textures
	//		if ((it)->snID == 1) { // tank top
	//			(it)->model = (it)->localRotateMtx = glm::mat4(1.0f);
	//			(it)->worldRotateMtx = glm::rotate((it)->localRotateMtx, angle, glm::vec3(0, 1, 0));
	//			// Update the localTransformMtx
	//			(it)->worldTransformMtx = (it)->worldRotateMtx *
	//				(it)->worldScaleMtx
	//				(it)->worldTranslateMtx;

	//			// Calculate the model matrix
	//			(it)->model = (it)->localTransformMtx(*it)->model;
	//		}
	//	}
	//}
