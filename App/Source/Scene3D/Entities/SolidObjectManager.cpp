...
bool CSolidObjectManager::CheckForCollision(void)
{
		...
		for (unsigned int i = 0; i < cProjectileManager->vProjectile.size(); i++)
		{
				...
				(cProjectileManager->vProjectile[i])->GetPosition()))
			{
				if ((*it)->GetType() == CSolidObject::TYPE::PLAYER)
				{
					...
					(*it)->SetHealth(health - (cProjectileManager->vProjectile[i])->GetDamage());
				...
				else if ((*it)->GetType() == CSolidObject::TYPE::NPC)
				{
					...
					if (health <= 0)
					{
						cplayer->GetWeapon()->SetTotalRound(cplayer->GetWeapon()->GetTotalRound() + 5);
						(*it)->SetStatus(false);
					}
				...
				(cProjectileManager->vProjectile[i])->GetPosition() + (cProjectileManager->vProjectile[i])->boxMax) == true)
			{
				if ((*it)->GetType() == CSolidObject::TYPE::PLAYER)
				{
					...
					bResult = true;
					//CCameraEffectsManager::GetInstance()->Get("HitMarker")->SetStatus(!bResult);
					break;
				}
				else if ((*it)->GetType() == CSolidObject::TYPE::NPC)
				{
	...
	if (bResult == true)
	{
		...
		CPlayer3D::GetInstance()->lasthit = 0;
		//int hp = cplayer->GetHealth() - 3;
		//cplayer->SetHealth(hp);
	}
...
void CSolidObjectManager::RenderMini(void)
{
	// Render all entities
	std::list<CSolidObject*>::iterator it, end;
	end = lSolidObject.end();
