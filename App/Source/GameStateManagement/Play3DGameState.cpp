...
bool CPlay3DGameState::Update(const double dElapsedTime)
{
	...
	else if (CPlayer3D::GetInstance()->bPlayerLost == true)
	{
		// Reset the CKeyboardController
		//CKeyboardController::GetInstance()->Reset();

		// Load the menu state
		cout << "Loading OverState" << endl;
		CGameStateManager::GetInstance()->SetActiveGameState("OverState");
	}
	else if (CPlayer3D::GetInstance()->bPlayerWon == true)
	{
		// Reset the CKeyboardController
		//CKeyboardController::GetInstance()->Reset();
