												...
												const float height)
{
	...
	v.position = glm::vec3(x - 0.5f * width, y - 0.5f * height, z);
	v.color = color;
	v.texCoord = glm::vec2(0, 0);
	vertex_buffer_data.push_back(v);
...
CMesh* CMeshBuilder::GenerateSphere(glm::vec4 color, unsigned numStacks, unsigned numSlices, float radius)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	for (unsigned stack = 0; stack < numStacks + 1; ++stack)
	{
		...
		for (unsigned slice = 0; slice < numSlices + 1; ++slice)
		{
			...
			float x = cosf(glm::radians(phi)) * cosf(glm::radians(theta));
			float y = sinf(glm::radians(phi));
			float z = cosf(glm::radians(phi)) * sinf(glm::radians(theta));

			v.position = glm::vec3(radius * x, radius * y, radius * z);	
			v.color = color;
			v.texCoord = glm::vec2(slice, stack); 
			vertex_buffer_data.push_back(v);
		}
	}
...
//CMesh* CMeshBuilder::GenerateQuad(glm::vec4 color, float lengthX, float lengthY)
//{
//	Vertex v;
//	std::vector<Vertex> vertex_buffer_data;
//	std::vector<unsigned> index_buffer_data;
//
//	v.position = glm::vec3(0.5f, 0.5f, 0.f);	v.color = color; v.texCoord = glm::vec2(1, 1); vertex_buffer_data.push_back(v);
//	v.position = glm::vec3(-0.5f, 0.5f, 0.f);	v.color = color; v.texCoord = glm::vec2(0, 1); vertex_buffer_data.push_back(v);
//	v.position = glm::vec3(-0.5f, -0.5f, 0.f);	v.color = color; v.texCoord = glm::vec2(0, 0); vertex_buffer_data.push_back(v);
//	v.position = glm::vec3(0.5f, -0.5f, 0.f);	v.color = color; v.texCoord = glm::vec2(1, 0); vertex_buffer_data.push_back(v);
//
//
//	index_buffer_data.push_back(0);
//	index_buffer_data.push_back(1);
